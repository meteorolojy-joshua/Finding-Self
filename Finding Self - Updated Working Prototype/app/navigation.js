...
window.NAV = (() => {
  ...
  function state() {
    ...
    const restores=[window.ROOM,window.NOTES,window.SCENARIO_UI].filter(Boolean).map(m=>m.navSnapshot?.()).filter(Boolean);
  ...
  function classify(root) {
    ...
    if(!group) {
      ...
      else if(/room-cue/.test(id)) group='cue';
      ...
      else if(/note-composer/.test(id)) group='compose';
      else if(/note-saved/.test(id)) group='saved-note';
      else if(/moments|notes/.test(id)) group='notebook';
      else group='check-in';
    }
  ...
  function init() {
    ...
    bar.innerHTML='<span class="app-identity"><img src="assets/brand-icon.svg" alt=""><span>Finding Self,<br>Author of Worlds</span></span><nav aria-label="Page navigation"><button type="button" data-nav="back">Back</button><button type="button" data-nav="exit">Exit</button><button type="button" data-nav="settings" aria-label="Settings"><img src="assets/settings-icon.svg" alt=""></button></nav>';
    ...
    window.addEventListener('click',e=>{
      ...
      if(n){e.preventDefault();e.stopImmediatePropagation();if(n.dataset.nav!=='settings'&&!settings&&window.NOTES?.navigationBusy?.()){window.EXPLORATIONS?.notify('Stop or cancel the recording before leaving this note.');return;}capture();if(n.dataset.nav==='back')back();if(n.dataset.nav==='exit')exit();if(n.dataset.nav==='settings')UI.renderSettings();return;}
