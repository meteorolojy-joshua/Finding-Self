...
const NOTES = (() => {
    ...
    const inner = sorted.map(it => {
      ...
      if (it.kind === 'photo') {
        ...
        return `<div class="note-item note-item-photo${selCls}${interactive ? '' : ' frozen'}" data-exid="${esc(it.exid)}" style="left:${it.x}%;top:${it.y}px;z-index:${it.z + 1};${it.wpct ? `width:${it.wpct}%` : ''}"
          ${interactive ? 'tabindex="0" role="button" aria-label="Photograph — press Enter to select"' : ''}>
      ...
      if (it.kind === 'audio') {
        return `<div class="note-item note-item-audio${selCls}${interactive ? '' : ' frozen'}" data-exid="${esc(it.exid)}" style="left:${it.x}%;top:${it.y}px;z-index:${it.z + 1}"
          ${interactive ? 'tabindex="0" role="button" aria-label="Recording — press Enter to select"' : ''}>
      ...
      return `<div class="note-item note-item-text${selCls}${interactive ? '' : ' frozen'}" data-exid="${esc(it.exid)}" style="left:${it.x}%;top:${it.y}px;z-index:${it.z + 1};color:${it.color || CAP_COLORS[1].value};--s:${it.s || 1}"
        ${interactive ? 'tabindex="0" role="button" aria-label="Words — press Enter to select and edit"' : ''} ${interactive ? 'contenteditable="false"' : ''} data-phtext="Write as little or as much as you want">${esc(it.text || '')}</div>`;
    }).join('');
  ...
  function selectedToolbar() {
    ...
    const kindTools = it.kind === 'text'
      ...
      : '';
    const wordsEdit = it.kind === 'text'
      ? `<div class="note-wordsedit">
          <label class="note-wordslabel" for="note-words">Words</label>
          <textarea id="note-words" class="note-words" rows="3" data-words-edit>${esc(it.text || '')}</textarea>
        </div>`
      : '';
    return `<div class="note-captools" role="group" aria-label="The chosen ${kindLabel} — ${it.kind === 'text' ? 'words, color and layering' : 'layering'}">
      ${wordsEdit}
      ${kindTools}
      ${zTools(it)}
      <button class="note-act" type="button" data-nact="remove-sel">Remove this ${kindLabel}</button>
    </div>`;
  }
  ...
  /* a new photo lands below the lowest item already on the board, so it
     never covers the words that are there; the board grows to meet it */
  function freeY(photoH, bw) {
    const bottomOf = (it) => {
      if (it.kind === 'photo' && it.wpct && it.w && it.h) return (it.y || 0) + (it.wpct / 100) * bw / (it.w / it.h);
      return (it.y || 0) + 90;
    };
    const lowest = draft.items.reduce((m, it) => Math.max(m, bottomOf(it)), 0);
    return Math.max(60, Math.round(lowest + 24));
  }
  function pickPhoto() {
  ...
  function onPhotoChosen(input) {
    ...
    reader.onload = () => {
      ...
      img.onerror = () => {
        ...
        const br = board ? board.getBoundingClientRect() : null;
        const bw = br && br.width ? br.width : 600;
        const it = addItem({ exid: exid(), kind: 'photo', x: 50, y: freeY(200, bw), wpct: 70, dataUrl: reader.result, w: 0, h: 0 });
        selId = it.exid;
        renderComposer();
      };
  ...
  /* the toolbar Words textarea is the editor for the selected text item;
     focusing it here (not the board item) keeps one editing surface */
  function focusWordsEditor() {
    const ta = $view().querySelector('#note-words');
    if (ta) ta.focus();
  }
  ...
  function endEdit(el) {
    ...
    if (it) it.text = text;
    /* blank words are kept as empty placeholders (shown with hint text) —
       deletion is always explicit: drag to the trash or use Remove */
  }
  function flushEdits() {
  ...
  /* the labeled words textarea: typing updates the draft live so Keep
     enables as you type; blur or Enter commits (trims trailing space) */
  function selectedTextItem() {
    const it = selectedItem();
    return (it && it.kind === 'text') ? it : null;
  }
  function updateKeepState() {
    const off = !hasContent() || busy();
    ['note-keep', 'note-share'].forEach(od => {
      const b = $view().querySelector(`[data-od-id="${od}"]`);
      if (b) { b.disabled = off; b.setAttribute('aria-disabled', String(off)); }
    });
  }
  function commitWordsEditor() {
    const ta = document.getElementById('note-words');
    const it = selectedTextItem();
    if (!ta || !it || !draft) return false;
    const committed = ta.value !== it.text;
  ...
  function onItemUp(e) {
    ...
    }
    {
      ...
      selId = ex;
      renderComposer();
      if (it && it.kind === 'text') focusWordsEditor();
    }
  }
  ...
  function init() {
    $view().addEventListener('click', (e) => {
      ...
      const acts = {
        ...
        'remove-sel': () => {
          ...
          if (!it || !draft) return;
          stopPlayer();
          draft.items = draft.items.filter(i => i.exid !== it.exid);
          selId = null;
          renderComposer();
        },
    ...
    $view().addEventListener('input', (e) => {
      if (!e.target || e.target.id !== 'note-words') return;
      const it = selectedTextItem();
      if (!it) return;
      it.text = e.target.value;
      // show the words on the board as they are typed, without re-rendering
      const el = $view().querySelector(`.note-board [data-exid="${it.exid}"]`);
      if (el) el.textContent = it.text;
      updateKeepState();
    });
    $view().addEventListener('keydown', (e) => {
      if (e.target && e.target.id === 'note-words' && e.key === 'Enter' && !e.shiftKey && !e.ctrlKey && !e.metaKey) {
        e.preventDefault();
    ...
    $view().addEventListener('focusout', (e) => {
      ...
      if (el) { endEdit(el); renderComposer(); return; }
      if (e.target && e.target.id === 'note-words') { commitWordsEditor(); updateKeepState(); }
    }, true);
    /* Keyboard: Enter / Space on a board item selects it, reaching the same
       editor and toolbar a pointer tap would. Inner controls (play button,
       words textarea) keep their own behavior. */
    $view().addEventListener('keydown', (e) => {
      if (e.key !== 'Enter' && e.key !== ' ') return;
      const itemEl = e.target.closest && e.target.closest('.note-item[data-exid][role="button"]');
      if (!itemEl || itemEl.classList.contains('frozen')) return;
      const inner = e.target.closest && e.target.closest('[data-nact][role="button"], textarea, input');
      if (inner && inner !== itemEl) return; // let the inner control's own handler run
      e.preventDefault();
      const ex = itemEl.getAttribute('data-exid');
      const it = draft && draft.items.find(i => i.exid === ex);
      if (!it) return;
      selId = ex;
      renderComposer();
      if (it.kind === 'text') focusWordsEditor();
      else {
        const again = $view().querySelector(`.note-board [data-exid="${ex}"]`);
