...
const ROOM = (() => {
  ...
  /* material chips: kept notes, attachable without summarizing */
  ...
  function renderExplore() {
    ...
    $view().innerHTML = `<div class="xpage" data-od-id="room-explore">
      ...
      <div class="xpage-col">
        ...
        ${clip}
        <button class="x-bring" type="button" data-ract="p-material-x">${X_CLIP_ICON}Bring an existing note</button>
        <div class="x-actions">
  ...
  function noticeSourceLabel() {
    const n = noteById(nDraft.sourceId);
    return n ? 'the note kept ' + day(n.savedAt) : 'a note';
  }
  ...
  function renderNoticeSaved(c) {
    $view().innerHTML = `<div class="card quiet" data-od-id="room-notice-saved">
      ${head('Kept in What helps.', 'Stored on this device', '“' + c.text + '” — the note it came from is unchanged.')}
      <div class="btnrow">
  ...
  /* the What helps collection page is retired: conditions are still noticed
     on notes and reachable through a kept thing's gather picker, but
  ...
  function renderGatherConditions(id) {
    ...
    $view().innerHTML = `<div class="card quiet" data-od-id="room-gather-c">
      ...
      ${list.length ? `<ul class="list">${list.map(c => `<li><button class="note-open" type="button" data-ract="g-add-condition" data-id="${esc(c.id)}" data-v="${esc(id)}"><b>${esc(c.text)}</b><span class="sub">${esc(c.kind || 'something')}</span></button></li>`).join('')}</ul>`
        : '<p class="muted">Nothing in What helps yet — “Notice what helped here” on a kept note adds to it.</p>'}
  ...
  function renderCuePreview(flowId) {
    ...
    $view().innerHTML = `<div class="card quiet" data-od-id="room-cue-preview">
      ...
      <p class="support">If you confirm, one answer — <b>“${esc(cueDraft.text)}”</b> — will be added to the first question of <b>${esc(s.title)}</b>. Choosing it in a run simply stays on that question. You can remove it later from Settings → Added answers.</p>
      <div class="btnrow">
  ...
  /* ---------- pickers: notes as material, links, attachments ---------- */
  let pickCtx = null;
  function renderPicker() {
    const notes = allNotes().slice().sort((a, b) => String(b.savedAt).localeCompare(String(a.savedAt)));
    const titles = { material: 'Bring an existing note', connect: 'Connect to another note', attach: 'Keep a note with this arrangement' };
    $view().innerHTML = `<div class="card quiet" data-od-id="room-picker">
      ${head(titles[pickCtx.purpose] || 'Choose', null,
        pickCtx.purpose === 'material' ? 'The note comes along as it is — no summary, no explanation.' :
      ...
      ${!notes.length ? '<p class="muted">No kept notes yet.</p>' : ''}
      ...
      const acts = {
        ...
        'cue-confirm': () => {
          ...
          $view().innerHTML = `<div class="card quiet" data-od-id="room-cue-done">
            ${head('The cue is placed.', null, '“' + cueDraft.text + '” now appears at the start of ' + s.title + '. The arrangement is unchanged; the cue can be removed later from Settings → Added answers.')}
