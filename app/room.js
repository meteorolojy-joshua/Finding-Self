/* ============================================================
   ...
   fsaw.conditions.v1, fsaw.noteLinks.v1), its own screens,
   ...
   - The source stays intact: practices can start from a note
     or marker without altering or summarizing it.
   ============================================================ */
'use strict';
...
const ROOM = (() => {
  ...
  function linkNotes(a, b) {
    ...
    const m = links();
    m[a] = m[a] || []; m[b] = m[b] || [];
    if (!m[a].includes(b)) m[a].push(b);
    if (!m[b].includes(a)) m[b].push(a);
    saveLinks(m);
  }

  /* notes' public surface (added in notes.js) */
  const noteById = (id) => (window.NOTES && NOTES.get) ? NOTES.get(id) : null;
  const allNotes = () => (window.NOTES && NOTES.all) ? NOTES.all() : [];
  const markers = () => ENGINE.lsGet(ENGINE.LS.marker, []);
  ...
  /* material chips: kept notes + markers, attachable without summarizing */
  function materialLabel(m) {
    if (m.type === 'note') { const n = noteById(m.id); return n ? 'a note kept ' + day(n.savedAt) : 'a note (since removed)'; }
    if (m.type === 'marker') { const k = markers().find(x => x.id === m.id); return k ? 'marker: “' + (k.label || 'Marker') + '”' : 'a marker (since removed)'; }
  ...
  function xNotePreview(n) {
    if (!n || !n.items) return '';
    const parts = [];
    for (const it of n.items) {
      if (it.type === 'text' && String(it.text || '').trim()) parts.push(it.text.trim());
      else if (it.type === 'photo') parts.push('[a photograph]');
      else if (it.type === 'audio') parts.push('[a recording]');
      if (parts.join(' ').length > 120) break;
    }
    const s = parts.join(' · ');
    return s.length > 130 ? s.slice(0, 128).replace(/\s+\S*$/, '') + '…' : s;
  }
  function xMaterialClip(material) {
    const rows = material.map(m => {
      const label = materialLabel(m);
      let preview = '';
      if (m.type === 'note') preview = xNotePreview(noteById(m.id));
      else if (m.type === 'marker') { const k = markers().find(x => x.id === m.id); preview = k && k.note ? String(k.note).trim() : ''; }
      return `<li class="x-clip-item"><span class="x-clip-title">${esc(label)}</span>${preview ? `<span class="x-clip-preview">${esc(preview)}</span>` : ''}</li>`;
    }).join('');
  ...
  function renderExplore() {
    ...
    $view().innerHTML = `<div class="xpage" data-od-id="room-explore">
      ...
      <div class="xpage-col">
        ...
        ${clip}
        <button class="x-bring" type="button" data-ract="p-material-x">${X_CLIP_ICON}Bring an existing note or marker</button>
        <div class="x-actions">
  ...
  /* ============================================================
     2 · Notice what helped here + the What helps collection
     ============================================================ */
  let nDraft = null;
  const NOTICE_KINDS = ['a person', 'a place or setting', 'a resource', 'an activity', 'an expectation', 'something else'];
  const NOTICE_EXAMPLES = ['I could leave when I wanted', 'Nobody needed me to explain', 'We were making something together'];

  function openNotice(sourceType, sourceId) {
    nDraft = { sourceType, sourceId, kind: '', text: '' };
    renderNotice();
  }
  function noticeSourceLabel() {
    if (nDraft.sourceType === 'note') { const n = noteById(nDraft.sourceId); return n ? 'the note kept ' + day(n.savedAt) : 'a note'; }
    const m = markers().find(x => x.id === nDraft.sourceId);
    return m ? 'the marker “' + (m.label || 'Marker') + '”' : 'a marker';
  }
  function renderNotice() {
    $view().innerHTML = `<div class="card quiet" data-od-id="room-notice">
      ${head('Notice what helped here', 'From ' + noticeSourceLabel(), 'Ease counts. Pleasure counts. Ordinary things count — this does not have to be about something difficult.')}
      <div class="room-chips" role="group" aria-label="What kind of thing helped">${NOTICE_KINDS.map(k => `<button class="fpill" type="button" data-ract="n-kind" data-v="${esc(k)}" aria-pressed="${nDraft.kind === k}">${esc(k)}</button>`).join('')}</div>
      <textarea id="room-notice" rows="3" placeholder="What made a difference?">${esc(nDraft.text)}</textarea>
      <div class="ghostrow">${NOTICE_EXAMPLES.map(x => `<button class="ghost" type="button" data-ract="n-example" data-v="${esc(x)}">${esc(x)}…</button>`).join('')}</div>
      <div class="btnrow">
        <button class="btn" type="button" data-ract="n-save">Keep this</button>
        <button class="note-act" type="button" data-ract="n-back">Not now</button>
      </div>
    </div>`;
    settle();
  }
  function renderNoticeSaved(c) {
    $view().innerHTML = `<div class="card quiet" data-od-id="room-notice-saved">
      ${head('Kept in What helps.', 'Stored on this device', '“' + c.text + '” — the note or marker it came from is unchanged.')}
      <div class="btnrow">
        <button class="btn" type="button" data-ract="p-from-condition" data-id="${esc(c.id)}">Make more room for this</button>
        <button class="note-act" type="button" data-ract="n-back">Back</button>
      </div>
    </div>`;
    settle();
  }

  /* the What helps collection page is retired: conditions are still noticed
     on notes/markers and reachable through a kept thing's gather picker, but
     there is no standalone browsing screen in the Thematic Explorations flow */

  /* ============================================================
     3 · Terms of belonging + the proposal editor
     ============================================================ */
  let bDraft = null;
  ...
  function renderArrangement(id, opts) {
    ...
    const attached = (a.attachedNotes || []).map(nid => noteById(nid)).filter(Boolean);
    ...
    if (editing) {
      ...
      body = `
        ...
        ${attached.length ? `<div class="room-viewfield"><span class="room-label">Notes kept with this</span><ul class="list">${attached.map(n => `<li><button class="ghost" type="button" data-ract="open-note" data-id="${esc(n.id)}">Open the note kept ${esc(day(n.savedAt))}</button></li>`).join('')}</ul></div>` : ''}
        ${gatheredHtml(a)}
        ${piecesHtml(a)}
        ...
        <div class="btnrow">
          ...
          <button class="note-act" type="button" data-ract="a-attach" data-id="${esc(a.id)}">Keep a note with this</button>
          <button class="note-act" type="button" data-ract="a-cue" data-id="${esc(a.id)}">Place a cue in a flow</button>
  ...
  function renderGather(id) {
    ...
    $view().innerHTML = `<div class="card quiet" data-od-id="room-gather">
      ...
      <div class="options">
        <button class="opt" type="button" data-ract="g-note" data-id="${esc(id)}"><b>A kept note</b></button>
  ...
  function renderGatherConditions(id) {
    ...
    $view().innerHTML = `<div class="card quiet" data-od-id="room-gather-c">
      ...
      ${list.length ? `<ul class="list">${list.map(c => `<li><button class="note-open" type="button" data-ract="g-add-condition" data-id="${esc(c.id)}" data-v="${esc(id)}"><b>${esc(c.text)}</b><span class="sub">${esc(c.kind || 'something')}</span></button></li>`).join('')}</ul>`
        : '<p class="muted">Nothing in What helps yet — “Notice what helped here” on a kept note or marker adds to it.</p>'}
  ...
  /* cue into an existing flow — with a preview before anything changes */
  let cueDraft = null;
  function renderCuePicker(id) {
    const a = getArrangement(id);
    if (!a) return goHome();
    cueDraft = { id, flowId: null, text: a.experiment || arrTitle(a) };
    const flows = ENGINE.PKG.starters || [];
    $view().innerHTML = `<div class="card quiet" data-od-id="room-cue">
      ${head('Place a cue in a flow', 'From: ' + arrTitle(a), 'A short reminder, added as an answer at the start of a flow you already use. You will preview the exact change before confirming.')}
      <label class="room-label" for="room-cue-text">The cue</label>
      <textarea id="room-cue-text" rows="2">${esc(cueDraft.text)}</textarea>
      <p class="support">Which flow should carry it?</p>
      <div class="options">${flows.map(s => `<button class="opt" type="button" data-ract="cue-flow" data-id="${esc(s.id)}"><b>${esc(s.title)}</b></button>`).join('')}</div>
      <div class="btnrow"><button class="note-act" type="button" data-ract="open-arrangement" data-id="${esc(id)}">Not now</button></div>
    </div>`;
    settle();
  }
  function renderCuePreview(flowId) {
    const a = getArrangement(cueDraft.id);
    const s = (ENGINE.PKG.starters || []).find(x => x.id === flowId);
    if (!a || !s) return goHome();
    cueDraft.flowId = flowId;
    cueDraft.text = val('room-cue-text') || cueDraft.text;
    $view().innerHTML = `<div class="card quiet" data-od-id="room-cue-preview">
      ${head('Preview the change', 'Nothing has changed yet', null)}
      <p class="support">If you confirm, one answer — <b>“${esc(cueDraft.text)}”</b> — will be added to the first question of <b>${esc(s.title)}</b> (<code>${esc(s.standard_entry_node)}</code>). Choosing it in a run simply stays on that question. It remains until you remove it in Settings → Delete everything, or among your added answers.</p>
      <div class="btnrow">
        <button class="btn" type="button" data-ract="cue-confirm">Confirm — place the cue</button>
        <button class="note-act" type="button" data-ract="a-cue" data-id="${esc(a.id)}">Back</button>
      </div>
    </div>`;
    settle();
  }

  /* ---------- pickers: notes & markers as material, links, attachments ---------- */
  let pickCtx = null;
  function renderPicker() {
    const notes = allNotes().slice().sort((a, b) => String(b.savedAt).localeCompare(String(a.savedAt)));
    const ms = markers();
    const titles = { material: 'Bring an existing note or marker', connect: 'Connect to another note', attach: 'Keep a note with this arrangement' };
    $view().innerHTML = `<div class="card quiet" data-od-id="room-picker">
      ${head(titles[pickCtx.purpose] || 'Choose', null,
        pickCtx.purpose === 'material' ? 'The note or marker comes along as it is — no summary, no explanation.' :
        pickCtx.purpose === 'connect' ? 'Connected notes sit alongside each other. Neither replaces the other; they can even disagree.' :
        'The note stays in your note-book too — this only links them.')}
      ${notes.length ? `<h2 class="h3">Kept notes</h2><ul class="list">${notes.map(n => `<li>
        <button class="note-open" type="button" data-ract="pick-note" data-id="${esc(n.id)}">
          <b>${esc(notesKindLabel(n))}</b><span class="sub">kept ${esc(day(n.savedAt))}</span>
          ${NOTES.boardHtml(n)}
        </button></li>`).join('')}</ul>` : ''}
      ${pickCtx.purpose === 'material' && ms.length ? `<h2 class="h3">Markers</h2><ul class="list">${ms.map(m => `<li>
        <button class="note-open" type="button" data-ract="pick-marker" data-id="${esc(m.id)}"><b>${esc(m.label || 'Marker')}</b><span class="sub">saved ${esc(day(m.savedAt))}</span></button></li>`).join('')}</ul>` : ''}
      ${!notes.length && !(pickCtx.purpose === 'material' && ms.length) ? '<p class="muted">No kept notes yet.</p>' : ''}
      <div class="btnrow"><button class="note-act" type="button" data-ract="pick-cancel">Back</button></div>
    </div>`;
    settle();
    if (window.NOTES && NOTES.settleBoards) NOTES.settleBoards($view());
  }
  function notesKindLabel(n) {
    const items = (n.items || []);
    const parts = [];
    if (items.some(i => i.kind === 'text' && (i.text || '').trim())) parts.push('words');
    if (items.some(i => i.kind === 'photo')) parts.push('a photograph');
    if (items.some(i => i.kind === 'audio')) parts.push('a recording');
    return 'A note — ' + (parts.join(', ') || 'empty');
  }
  ...
  /* ---------- related notes (links + versions), rendered inside the saved-note screen ---------- */
  function relatedHtml(note) {
    const m = links();
    const ids = new Set(m[note.id] || []);
    allNotes().forEach(n => {
      if (n.versionOf === note.id || note.versionOf === n.id) ids.add(n.id);
      if (note.versionOf && n.versionOf === note.versionOf) ids.add(n.id);
    });
    ids.delete(note.id);
    const rel = [...ids].map(noteById).filter(Boolean);
    if (!rel.length) return '';
    return `<div class="room-related"><span class="room-label">Kept alongside</span><ul class="list">${rel.map(n =>
      `<li><button class="ghost" type="button" data-ract="open-note" data-id="${esc(n.id)}">${esc(notesKindLabel(n))}, kept ${esc(day(n.savedAt))}</button></li>`).join('')}</ul></div>`;
  }

  /* ============================================================
     events
     ============================================================ */
  function openPracticeFromCondition(cid) {
    const c = conditions().find(x => x.id === cid);
    if (!c) return;
    openPractice({ material: [{ type: 'condition', id: c.id }], help: (c.kind ? c.kind + ': ' : '') + c.text });
  }
  ...
  function init() {
    $view().addEventListener('click', (e) => {
      ...
      const acts = {
        ...
        'p-from-condition': () => { roomBackFn = () => renderArrangement(id); openPracticeFromCondition(id); },
        'lang-open': () => openLang(),
        ...
        /* notice what helped */
        'notice-note': () => openNotice('note', id),
        'notice-marker': () => openNotice('marker', id),
        'n-kind': () => { nDraft.text = val('room-notice'); nDraft.kind = nDraft.kind === v ? '' : v; renderNotice(); },
        'n-example': () => { const t = document.getElementById('room-notice'); if (t) { t.value = v; t.focus(); } },
        'n-save': () => {
          const text = val('room-notice');
          if (!text) return;
          const c = { id: uid('cond'), text, kind: nDraft.kind, sourceType: nDraft.sourceType, sourceId: nDraft.sourceId, clusterId: null, createdAt: today() };
          const l = conditions(); l.push(c); saveConditions(l);
          renderNoticeSaved(c);
        },
        'n-back': () => {
          if (nDraft && nDraft.sourceType === 'note' && window.NOTES && NOTES.openSaved) NOTES.openSaved(nDraft.sourceId);
          else if (window.UI && UI.renderMoments) UI.renderMoments();
        },
        /* belonging */
        ...
        'a-attach': () => { pickCtx = { purpose: 'attach', arrangementId: id }; renderPicker(); },
        'a-cue': () => renderCuePicker(id),
        'r-outcome': (el) => {
          rDraft.madeRoom = v;
        ...
        'cue-flow': () => renderCuePreview(id),
        'cue-confirm': () => {
          const a = getArrangement(cueDraft.id);
          const s = (ENGINE.PKG.starters || []).find(x => x.id === cueDraft.flowId);
          if (!a || !s) return goHome();
          ENGINE.persistAddedAnswer(s.standard_entry_node, {
            optionId: 'USER.CUE.' + Date.now().toString(36),
            label: 'Cue: ' + cueDraft.text,
            answerSourceId: a.id,
            destination: s.standard_entry_node,
            semanticTag: 'cue', stateWrites: [], controlIntent: 'NONE'
          });
          $view().innerHTML = `<div class="card quiet" data-od-id="room-cue-done">
            ${head('The cue is placed.', null, '“' + cueDraft.text + '” now appears at the start of ' + s.title + '. The arrangement is unchanged; the cue can be removed with your other added answers.')}
            <div class="btnrow"><button class="btn" type="button" data-ract="open-arrangement" data-id="${esc(a.id)}">Back to the arrangement</button></div>
          </div>`;
          settle();
        },
        /* open explorations */
        'open-explore': () => openExplore(),
        'p-material-x': () => { pDraft.whatMatters = val('room-x1'); pDraft.draws = val('room-x2'); pickCtx = { purpose: 'material', returnTo: 'explore' }; renderPicker(); },
        ...
        'g-note': () => { pickCtx = { purpose: 'attach', arrangementId: id }; renderPicker(); },
        ...
        /* pickers */
        'pick-note': () => {
          if (pickCtx.purpose === 'material') {
            pDraft.material.push({ type: 'note', id });
            if (pickCtx.returnTo === 'explore') renderExplore(); else renderPractice(pDraft.step);
          }
          else if (pickCtx.purpose === 'connect') {
            linkNotes(pickCtx.fromNoteId, id);
            if (window.NOTES && NOTES.openSaved) NOTES.openSaved(pickCtx.fromNoteId);
          } else if (pickCtx.purpose === 'attach') {
            const a = getArrangement(pickCtx.arrangementId);
            if (a) { a.attachedNotes = a.attachedNotes || []; if (!a.attachedNotes.includes(id)) a.attachedNotes.push(id); putArrangement(a); }
            renderArrangement(pickCtx.arrangementId);
          }
        },
        'pick-marker': () => {
          pDraft.material.push({ type: 'marker', id });
          if (pickCtx.returnTo === 'explore') renderExplore(); else renderPractice(pDraft.step);
        },
        'pick-cancel': () => {
          if (pickCtx.purpose === 'material') { if (pickCtx.returnTo === 'explore') renderExplore(); else renderPractice(pDraft.step); }
          else if (pickCtx.purpose === 'connect') { if (window.NOTES && NOTES.openSaved) NOTES.openSaved(pickCtx.fromNoteId); }
          else renderArrangement(pickCtx.arrangementId);
        },
        /* from a saved note */
        'open-note': () => { if (window.NOTES && NOTES.openSaved) NOTES.openSaved(id); },
        'room-from-note': () => openPractice({ material: [{ type: 'note', id }] }),
        'connect-note': () => { pickCtx = { purpose: 'connect', fromNoteId: id }; renderPicker(); },
        'version-note': () => {
          const n = noteById(id);
          if (n && window.NOTES) NOTES.openComposer(NOTES.itemsOf(n), { versionOf: n.versionOf || n.id });
        },
  ...
  function navSnapshot() { const saved = structuredClone({pDraft,nDraft,bDraft,cDraft,rDraft,retDraft,sDraft,cueDraft,pickCtx,leafCat,extraLeaves,langCat,rsEditId}); return () => { ({pDraft,nDraft,bDraft,cDraft,rDraft,retDraft,sDraft,cueDraft,pickCtx,leafCat,extraLeaves,langCat,rsEditId} = structuredClone(saved)); }; }
  return { navSnapshot, init, relatedHtml, openPractice, openNotice, openLampRoom,
