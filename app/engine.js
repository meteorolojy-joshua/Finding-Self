...
/* ---------- elemental rooms ---------- */
/* Kept entries: [{id, element:'fury'|'grief', prompt, text, altar:[{obj,x,y}], keptAt}].
   Positions are percentages of the room field. Nothing is kept unless chosen. */
const Elemental = {
  _valid(e) {
    return e && typeof e.id === 'string' && (e.element === 'fury' || e.element === 'grief') && typeof e.keptAt === 'string';
  },
  _cleanAltar(altar) {
  ...
  getEntries() {
    ...
    if (!Array.isArray(raw)) return [];
    return raw.filter(Elemental._valid).sort((a, b) => new Date(b.keptAt) - new Date(a.keptAt));
  },
  getEntry(id) { return Elemental.getEntries().find(e => e.id === id) || null; },
  /* Returns the kept entry, or null when there is nothing to keep (keep-rule). */
  keep(entry) {
    const clean = {
      id: typeof entry.id === 'string' && entry.id ? entry.id : ('el-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 8)),
      element: entry.element === 'grief' ? 'grief' : 'fury',
    ...
    lsSet(LS.elemental, all);
    return clean;
  },
  remove(id) { lsSet(LS.elemental, Elemental.getEntries().filter(e => e.id !== id)); },
};
...
/* ---------- sampler: a stitched record of completed practices ----------
   ...
   Store: fsaw.sampler.v1 -> [{id, practiceId, at, season, runId, pattern}]
   pattern is the user's drawn stitch: {strokes:[{color, pts:[[x,y]...]}]}.
...
const Sampler = {
  ...
  /* A drawn pattern: {strokes:[{color:'#rrggbb', pts:[[x,y],...]}]} in 0..100
     space. Entries from before drawn stitches have no pattern; they render
     with the legacy practice motif. */
  cleanPattern(pattern) {
    if (!pattern || !Array.isArray(pattern.strokes)) return null;
    const strokes = [];
    for (const st of pattern.strokes.slice(0, 60)) {
      if (!st || typeof st.color !== 'string' || !/^#[0-9a-fA-F]{6}$/.test(st.color)) continue;
      if (!Array.isArray(st.pts)) continue;
      const pts = [];
      for (const p of st.pts.slice(0, 300)) {
        if (!Array.isArray(p) || typeof p[0] !== 'number' || typeof p[1] !== 'number'
            || !isFinite(p[0]) || !isFinite(p[1])) continue;
        pts.push([Math.min(100, Math.max(0, p[0])), Math.min(100, Math.max(0, p[1]))]);
      }
      if (pts.length) strokes.push({ color: st.color.toLowerCase(), pts });
    }
    return strokes.length ? { strokes } : null;
  },
  /* Render a drawn pattern as boxless stitch art: running-stitch dashes in
     the user's thread colours, directly placeable on the cloth. Pure. */
  patternSVG(pattern) {
    const clean = this.cleanPattern(pattern);
    if (!clean) return '';
    const parts = clean.strokes.map(st => {
      if (st.pts.length === 1) {
        const p = st.pts[0];
        return `<circle cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="3.4" fill="${st.color}" stroke="none"/>`;
      }
      const d = st.pts.map((p, i) => (i ? 'L' : 'M') + p[0].toFixed(1) + ' ' + p[1].toFixed(1)).join('');
      return `<path d="${d}" fill="none" stroke="${st.color}" stroke-width="5" stroke-linecap="round" stroke-linejoin="round" stroke-dasharray="7 5"/>`;
    }).join('');
    return `<svg viewBox="0 0 100 100" aria-hidden="true">${parts}</svg>`;
  },
  addStitch(practiceId, runId, at, pattern) {
...
function exitFlow() {
  ...
  // a practice run exits back to the Practice library (the hub), not home
  const wasPractice = !!Session.practiceId;
  newRun();
  if (wasPractice) return UI.renderPracticeLibrary();
  return renderHome();
}
