...
const UI = (() => {
  ...
  function controlRow(node) {
    ...
    if (controls.some(c => /wrong question/i.test(c)))
      parts.push(`<button class="pctl" type="button" data-act="wrong">This doesn\u2019t fit</button>`);
  ...
  function displayLabel(o) {
    if (o.semantic_tag === 'inventory') return ICO_MORE + 'Choose from more options';
  ...
  const NODE_TEXT = {
    'MM.00': {
      prompt: 'Notice yourself - what you did, how you felt, or something else',
      support: 'A note can be just a small pointer for later. You do not need to explain what happened.'
    }
  };
  function nodeText(node, field) {
    const o = NODE_TEXT[node.id];
    return (o && o[field]) || null;
  }

  const NOTE_WORLDS = [['W1', 'W1 · Inner'], ['W2', 'W2 · Embodied & personal'], ['W3', 'W3 · Relational / small-room'], ['W4', 'W4 · Wider / public']];
  let _noteWorldBlank = null;   // world code currently sitting in the sentence blank
  let _noteWorldDrag = null;    // { code, from: 'box' | 'blank' } while a drag is active
  function renderNoteWorld() {
    const host = document.getElementById('noteworld');
    if (!host) return;
    const W = window.ENGINE.WORLDS;
    const chipHtml = (code) => {
      const rec = NOTE_WORLDS.find(w => w[0] === code) || ['', code];
      const src = _noteWorldBlank === code ? 'blank' : 'box';
      return `<button class="world-chip has-cursor-tip" type="button" draggable="true" data-world="${esc(code)}" data-world-src="${src}" data-tip="drag into the blank" style="background:${(W[code] && W[code].color) || '#59636E'}">${esc(rec[1])}</button>`;
    };
    host.innerHTML = `
      <span class="noteworld-text">The part of me that did or felt this existed in my <span class="noteworld-blank${_noteWorldBlank ? ' filled' : ''}" data-world-drop><span class="noteworld-hint">drag an option here</span>${_noteWorldBlank ? chipHtml(_noteWorldBlank) : ''}</span> world.</span>
      <div class="worldbox" aria-label="World options — drag one into the blank">${NOTE_WORLDS.filter(w => w[0] !== _noteWorldBlank).map(([code]) => chipHtml(code)).join('')}</div>`;
  }
  function renderNoteComposer(node) {
    const promptText = nodeText(node, 'prompt');
    const supportText = nodeText(node, 'support');
    $view().innerHTML = `
      <div class="card quiet" data-od-id="node-${esc(node.id)}">
        ...
        <div class="metarow">
          <p class="support">${esc(supportText)}</p>
        </div>
        <div class="field"><label for="note-text">Your note</label>
          <div class="notewrap">
            <textarea id="note-text" rows="7" placeholder="Write as little or much as you want"></textarea>
            <div class="noteworld" id="noteworld" role="group" aria-label="Which world is this note about? Drag an option into the blank."></div>
          </div>
        </div>
        <div class="noterow">
          <button class="btn2" type="button" data-act="note-help">Help me find language</button>
          <button class="btn" type="button" data-act="note-submit">Finish</button>
        </div>
        <div id="note-help" hidden>
          <p class="muted small">Optional starters — use only if they fit:</p>
          <div class="ghostrow">
            <button class="ghost" type="button" data-act="note-append" data-note-add="I noticed that ">I noticed that…</button>
            <button class="ghost" type="button" data-act="note-append" data-note-add="What stood out was ">What stood out was…</button>
            <button class="ghost" type="button" data-act="note-append" data-note-add="I felt ">I felt…</button>
            <button class="ghost" type="button" data-act="note-append" data-note-add="What I did was ">What I did was…</button>
          </div>
        </div>
      </div>`;
    renderNoteWorld();
  ...
  function renderNode(node) {
    if (node.id === 'MM.00') { renderNoteComposer(node); return; }
        ...
        <div class="home-sec home-sec-entries" data-od-id="other-checkin-options">
          ...
          <div class="entry-shelf">
            <div class="options entry-list">
              ...
              <button class="opt entry-row ledge has-cursor-tip" type="button" data-act="entry" data-entry="ENTRY.TOOLKIT" data-tip="Change your scenarios, questions, and answers — always previewed and reversible"><span class="entry-ico" aria-hidden="true">${ICO_PLUS}</span><b>Edit/add scenarios</b></button>
            </div>
    ...
    $view().innerHTML = `
      <div class="card" data-od-id="starter-${esc(id)}">
        ...
        <dl class="kv">
          ...
          <dt>Marker behavior</dt><dd>${esc(s.marker_behavior)}</dd>
  ...
  function renderTerminal(dest) {
    const bodies = {
      ...
      'MOMENTS': ['Moments', 'Saved markers appear here.'],
      ...
      'MARKER.UNDO.EXIT': ['Marker removed.', 'Nothing else changed.']
    };
    ...
    if (dest.id === 'MARKER.UNDO.EXIT') {
      const markers = ENGINE.lsGet(ENGINE.LS.marker, []);
      if (markers.length) ENGINE.undoMarker(markers[markers.length - 1].id);
    }
  ...
  let _notesSort = { mode: 'stored', from: '', to: '', world: 'W*' };
  function renderMoments() {
    const markers = ENGINE.lsGet(ENGINE.LS.marker, []);
    const s = _notesSort;
    let list = markers.slice();
    if (s.mode === 'newest') {
      list.sort((a, b) => String(b.savedAt || '').localeCompare(String(a.savedAt || '')));
    } else if (s.mode === 'shuffle') {
      for (let i = list.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        const t = list[i]; list[i] = list[j]; list[j] = t;
      }
    }
    if (s.from) list = list.filter(m => String(m.savedAt || '').slice(0, 10) >= s.from);
    if (s.to) list = list.filter(m => String(m.savedAt || '').slice(0, 10) <= s.to);
    if (s.world && s.world !== 'W*') list = list.filter(m => (m.world || 'W*') === s.world);
    const worlds = [['W1', 'W1 · Inner world'], ['W2', 'W2 · Embodied & personal'], ['W3', 'W3 · Relational / small-room'], ['W4', 'W4 · Wider / public'], ['W*', 'Any world']];
    $view().innerHTML = `
      <div class="card" data-od-id="moments">
        ...
        <div class="sortrow" role="group" aria-label="Sort and filter notes">
          <span class="sortlabel">Sort by:</span>
          <button class="sortbtn" type="button" data-act="notes-sort" data-mode="shuffle" aria-pressed="${s.mode === 'shuffle'}">Shuffle (Random)</button>
          <button class="sortbtn" type="button" data-act="notes-sort" data-mode="newest" aria-pressed="${s.mode === 'newest'}">Newest</button>
          <label class="sortlabel" for="notes-from">From date</label>
          <input type="date" id="notes-from" data-notes-filter="from" value="${esc(s.from)}">
          <span class="sortlabel" aria-hidden="true">to</span>
          <label class="sr-only" for="notes-to">To date</label>
          <input type="date" id="notes-to" data-notes-filter="to" value="${esc(s.to)}">
          <label class="sortlabel" for="notes-world">World</label>
          <select id="notes-world" data-notes-filter="world">${worlds.map(([code, label]) => `<option value="${code}"${s.world === code ? ' selected' : ''}>${esc(label)}</option>`).join('')}</select>
        </div>
        ${list.length ? `<ul class="list notes-list">${list.map(m => `
          <li><b>${esc(m.label || 'Marker')}</b>
          <span class="sub">${esc(m.returnPreference || 'no return set')} · saved ${esc((m.savedAt || '').slice(0, 10))}</span>
          <div class="ghostrow"><button class="ghost" type="button" data-act="marker-practice" data-marker="${esc(m.id)}">Open a practice with this</button>${window.ROOM ? `<button class="ghost" type="button" data-ract="notice-marker" data-id="${esc(m.id)}">Notice what helped here</button>` : ''}<button class="ghost" type="button" data-act="marker-delete" data-marker="${esc(m.id)}">Remove</button></div></li>`).join('')}</ul>`
        : (markers.length
          ? '<p class="muted">No notes match this sort or filter. A marker is a pointer you create — never a narrative you owe.</p>'
          : '<div class="collage-keepsake" aria-hidden="true"><i class="k1"></i><i class="k2"></i><i class="k3"></i><i class="k4"></i></div><p class="muted">No markers saved. A marker is a pointer you create — never a narrative you owe.</p>')}
  ...
  function updateInspector() {
    ...
    const rows = [
      ...
      ['markers (persisted)', ENGINE.lsGet(ENGINE.LS.marker, []).length],
      ['patches (persisted)', ENGINE.lsGet(ENGINE.LS.patch, []).length],
      ['fallback hit', S.currentNodeId === 'ROUTE.INVALID' ? 'ROUTE.INVALID' : (S.currentNodeId === 'ROUTE.REPAIR' ? 'ROUTE.REPAIR' : 'none')]
    ];
  ...
  function init() {
    ...
    $view().addEventListener('click', (e) => {
      ...
      const acts = {
        ...
        'wrong': () => ENGINE.routeRepair('user rejected the question frame'),
        ...
        'marker-practice': () => {
          ENGINE.newRun(); S.customText['__marker'] = btn.getAttribute('data-marker');
          ENGINE.gotoNode('PM.03');
        },
        'marker-delete': () => { ENGINE.undoMarker(btn.getAttribute('data-marker')); renderMoments(); },
        'notes-sort': () => {
          const mode = btn.getAttribute('data-mode');
          _notesSort.mode = (_notesSort.mode === mode) ? 'stored' : mode;
          renderMoments();
        },
        'sim-fire': () => { ENGINE.newRun(); ENGINE.simulateTrigger(btn.getAttribute('data-label')); },
        'export-data': () => {
          const blob = new Blob([JSON.stringify({
            markers: ENGINE.lsGet(ENGINE.LS.marker, []),
        ...
        'note-submit': () => {
          ...
          if (val) {
            const rec = { label: val };
            if (_noteWorldBlank) rec.world = _noteWorldBlank;
            ENGINE.saveMarker(rec);
            _noteWorldBlank = null;
          }
          ENGINE.Session.noteWorld = null;
          renderMoments();
        },
        'note-append': () => {
          const t = document.getElementById('note-text');
          if (t) { t.value += (t.value && !t.value.endsWith(' ') ? ' ' : '') + btn.getAttribute('data-note-add'); t.focus(); }
        }
      };
    ...
    $view().addEventListener('change', (e) => {
      const key = e.target.getAttribute && e.target.getAttribute('data-notes-filter');
      if (!key) return;
      _notesSort[key] = e.target.value;
      renderMoments();
      const el = document.getElementById(e.target.id);
      if (el) el.focus();
    });

    /* note composer: drag world options between the box and the sentence blank */
    $view().addEventListener('dragstart', (e) => {
      const chip = e.target.closest && e.target.closest('.world-chip[data-world]');
      if (!chip) return;
      const src = chip.getAttribute('data-world-src') || 'box';
      _noteWorldDrag = { code: chip.getAttribute('data-world'), from: src };
      e.dataTransfer.setData('text/plain', _noteWorldDrag.code);
      e.dataTransfer.effectAllowed = 'move';
      if (src === 'box') {
        // nothing left behind at the origin; the remaining options reflow at once
        setTimeout(() => { if (_noteWorldDrag) chip.classList.add('drag-gone'); }, 0);
      } else {
        // the blank and its faint hint reappear the moment the option leaves it
        // (deferred so hiding the source doesn't abort the drag itself)
        const blank = chip.closest('.noteworld-blank');
        setTimeout(() => { if (_noteWorldDrag && blank) blank.classList.add('sourcing'); }, 0);
      }
    });
    $view().addEventListener('dragover', (e) => {
      if (!_noteWorldDrag) return;
      const blank = e.target.closest && e.target.closest('.noteworld-blank');
      if (blank) {
        if (_noteWorldDrag.from === 'blank' && _noteWorldBlank === _noteWorldDrag.code) return;
        e.preventDefault();
        e.dataTransfer.dropEffect = 'move';
        blank.classList.add('over');
        return;
      }
      const box = e.target.closest && e.target.closest('.worldbox');
      if (box) {
        e.preventDefault();
        e.dataTransfer.dropEffect = 'move';
    ...
    $view().addEventListener('dragleave', (e) => {
      const blank = e.target.closest && e.target.closest('.noteworld-blank');
      if (blank && !(e.relatedTarget && blank.contains(e.relatedTarget))) blank.classList.remove('over');
      const box = e.target.closest && e.target.closest('.worldbox');
      if (box && !(e.relatedTarget && box.contains(e.relatedTarget))) box.classList.remove('over');
    });
    $view().addEventListener('drop', (e) => {
      ...
      if (!blank && !box) return;
      e.preventDefault();
      const drag = _noteWorldDrag;
      _noteWorldDrag = null;
      if (blank) {
        if (drag.from === 'blank' && drag.code === _noteWorldBlank) return;
        _noteWorldBlank = drag.code; // any occupant is displaced back to the box
      } else if (box) {
        _noteWorldBlank = null; // dropped back in the box: returns to its original spot
      }
      ENGINE.Session.noteWorld = _noteWorldBlank;
      renderNoteWorld();
    });
    $view().addEventListener('dragend', () => {
      _noteWorldDrag = null;
      document.querySelectorAll('.noteworld-blank.sourcing').forEach(el => el.classList.remove('sourcing'));
      document.querySelectorAll('.worldbox.over, .noteworld-blank.over').forEach(el => el.classList.remove('over'));
      document.querySelectorAll('.world-chip.drag-gone').forEach(el => el.classList.remove('drag-gone'));
    });
  ...
  return {
    ...
    renderPrimitiveTerminal, renderOnboardingResult, renderNode,
