...
const UI = (() => {
  ...
  const ICO_CUT = '<svg class="opt-ico" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="6" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><path d="M8.5 8.5 20 20M8.5 15.5 20 4"/></svg>';
        ...
        <div class="home-sec home-sec-entries" data-od-id="other-checkin-options">
          ...
          <div class="entry-shelf">
            <div class="options entry-list">
              <button class="opt entry-row ledge has-cursor-tip" type="button" data-act="practice-library" data-tip="Your sampler and your practices — rehearse for later, or gather the small things of a day"><span class="entry-ico" aria-hidden="true">${ICO_ARM}</span><b>Practices</b></button>
  ...
  function renderTerminal(dest) {
    ...
    $view().innerHTML = `
      <div class="card" data-od-id="terminal-${esc(dest.id)}">
        ...
        <div class="btnrow">
          ${dest.id === 'PRACTICE.COMPLETE'
            ? '<button class="btn" type="button" data-act="practice-library">Practice library</button>'
    ...
    $view().innerHTML = `
      <div class="card" data-od-id="practice-library">
        ...
        <div class="home-sec" data-od-id="practice-sampler">
          ...
          <p class="support">${esc(cur.cloth)} — each completed practice can add one stitch, drawn by you. Tap a stitch to revisit it.</p>
  ...
  function stComposerHTML() {
    return `<form class="st-composer" id="st-composer-form" autocomplete="off">
      ...
      ${ST_composerPhoto
        ? `<span class="st-pending"><img class="st-thumb" src="${ST_composerPhoto}" alt="Photo ready to attach"><button class="st-iconbtn" type="button" data-act="smallthings-cut-composer" aria-label="Cut out a shape from this photo">${ICO_CUT}</button><button class="st-iconbtn st-remove" type="button" data-act="smallthings-photo-clear" aria-label="Remove this photo">×</button></span>`
  ...
  function stZoom(id) {
    ...
    ov.innerHTML = `
      <div class="samp-dialog st-viewer">
        ...
        <div class="btnrow">${editable ? `<button class="btn2" type="button" data-act="smallthings-cut-entry" data-id="${esc(e.id)}">Cut out a shape</button>` : ''}<button class="ghost" type="button" data-act="smallthings-zoom-close">Close</button></div>
      </div>`;
    ov.addEventListener('click', ev => { if (ev.target === ov) ov.remove(); });
    $view().appendChild(ov);
  }
  /* ---------- photo scissors: cut a shape out of a photo ---------- */
  /* The photo stays whole unless the user closes a cut loop — "Keep the
     whole photo" (or closing the dialog) leaves the original untouched. */
  const CUT_LOOP_MIN_PTS = 24;
  const CUT_CLOSE_RATIO = 0.045;

  function openPhotoCutter(dataUrl, target) {
    closePhotoCutter();
    const img = new Image();
    img.onload = () => {
      const MAXC = 900;
      const scale = Math.min(1, MAXC / Math.max(img.width, img.height));
      const W = Math.max(1, Math.round(img.width * scale));
      const H = Math.max(1, Math.round(img.height * scale));
      const c = UI._cutter = { img, W, H, path: [], drawing: false, target, canvas: null, scissors: null };
      const ov = document.createElement('div');
      ov.className = 'samp-overlay';
      ov.setAttribute('role', 'dialog');
      ov.setAttribute('aria-label', 'Cut out a shape');
      ov.innerHTML = `
        <div class="samp-dialog cut-dialog">
          <div class="samp-dialog-title">Cut out a shape</div>
          <p class="support">Drag the little scissors around the part you want to keep. You can lift your finger and carry on — the cut finishes by itself once your line closes into a loop.</p>
          <div class="cut-wrap">
            <canvas class="cut-canvas" width="${W}" height="${H}" aria-label="Photo to cut"></canvas>
            <span class="cut-scissors" aria-hidden="true">${ICO_CUT}</span>
          </div>
          <div class="btnrow">
            <button class="ghost" type="button" data-act="cutter-clear">Start over</button>
            <button class="ghost" type="button" data-act="cutter-keep-whole">Keep the whole photo</button>
          </div>
        </div>`;
      ov.addEventListener('click', e => { if (e.target === ov) closePhotoCutter(); });
      $view().appendChild(ov);
      c.canvas = ov.querySelector('.cut-canvas');
      c.scissors = ov.querySelector('.cut-scissors');
      wireCutter(c);
      cutterDraw(c);
    };
    img.onerror = () => stNotify('That photo could not be opened for cutting.');
    img.src = dataUrl;
  }

  function closePhotoCutter() {
    const ovs = $view().querySelectorAll('.samp-overlay');
    ovs.forEach(ov => { if (ov.querySelector('.cut-dialog')) ov.remove(); });
    UI._cutter = null;
  }

  function cutterClearPath() {
    const c = UI._cutter;
    if (!c) return;
    c.path = []; c.drawing = false;
    cutterDraw(c);
  }

  function cutterLoopClosed(c) {
    if (c.path.length < CUT_LOOP_MIN_PTS) return false;
    const s = c.path[0], p = c.path[c.path.length - 1];
    return Math.hypot(p[0] - s[0], p[1] - s[1]) < c.W * CUT_CLOSE_RATIO;
  }

  function cutterDraw(c) {
    const ctx = c.canvas.getContext('2d');
    ctx.clearRect(0, 0, c.W, c.H);
    ctx.drawImage(c.img, 0, 0, c.W, c.H);
    if (c.path.length < 2) return;
    ctx.lineJoin = 'round'; ctx.lineCap = 'round';
    const trace = (style, dash, width) => {
      ctx.strokeStyle = style; ctx.setLineDash(dash); ctx.lineWidth = width;
      ctx.beginPath();
      c.path.forEach((p, i) => { if (i) ctx.lineTo(p[0], p[1]); else ctx.moveTo(p[0], p[1]); });
      ctx.stroke();
    };
    trace('rgba(255,255,255,.92)', [], Math.max(3, c.W * 0.012));
    trace('rgba(40,30,25,.88)', [10, 7], Math.max(2, c.W * 0.006));
    ctx.setLineDash([]);
    const s = c.path[0];
    ctx.fillStyle = 'rgba(255,255,255,.95)';
    ctx.beginPath(); ctx.arc(s[0], s[1], Math.max(4, c.W * 0.012), 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = 'rgba(40,30,25,.9)';
    ctx.beginPath(); ctx.arc(s[0], s[1], Math.max(2, c.W * 0.006), 0, Math.PI * 2); ctx.fill();
  }

  function wireCutter(c) {
    const cv = c.canvas;
    const toImg = (e) => {
      const r = cv.getBoundingClientRect();
      return [
        Math.min(c.W - 1, Math.max(0, (e.clientX - r.left) / r.width * c.W)),
        Math.min(c.H - 1, Math.max(0, (e.clientY - r.top) / r.height * c.H))
      ];
    };
    const moveScissors = (e) => {
      const wrap = cv.parentElement.getBoundingClientRect();
      c.scissors.style.left = (e.clientX - wrap.left) + 'px';
      c.scissors.style.top = (e.clientY - wrap.top) + 'px';
      c.scissors.style.opacity = '1';
    };
    cv.addEventListener('pointerdown', (e) => {
      ...
      if (!last || Math.hypot(p[0] - last[0], p[1] - last[1]) > c.W * 0.08) c.path = [p];
      c.drawing = true;
      moveScissors(e);
      cutterDraw(c);
    });
    cv.addEventListener('pointermove', (e) => {
      ...
      const p = toImg(e);
      const last = c.path[c.path.length - 1];
      if (last && Math.hypot(p[0] - last[0], p[1] - last[1]) < 2.5) return;
      c.path.push(p);
      cutterDraw(c);
      if (cutterLoopClosed(c)) cutterFinish(c);
    });
    const stop = () => { c.drawing = false; };
    cv.addEventListener('pointerup', stop);
    cv.addEventListener('pointercancel', stop);
    cv.addEventListener('pointerleave', () => { c.scissors.style.opacity = '0'; });
  }

  function cutterFinish(c) {
    if (!c || c.path.length < 3) return;
    c.drawing = false;
    ...
    maxX = Math.min(c.W, maxX + pad); maxY = Math.min(c.H, maxY + pad);
    const bw = maxX - minX, bh = maxY - minY;
    if (bw < 8 || bh < 8) { cutterClearPath(); return; }
    const MAXOUT = 600;
    const sc = Math.min(1, MAXOUT / Math.max(bw, bh));
    const out = document.createElement('canvas');
    out.width = Math.max(1, Math.round(bw * sc));
    out.height = Math.max(1, Math.round(bh * sc));
    const o = out.getContext('2d');
    o.scale(sc, sc);
    o.translate(-minX, -minY);
    o.beginPath();
    c.path.forEach((p, i) => { if (i) o.lineTo(p[0], p[1]); else o.moveTo(p[0], p[1]); });
    o.closePath();
    o.clip();
    o.drawImage(c.img, 0, 0, c.W, c.H);
    const url = out.toDataURL('image/png');
    if (c.target.kind === 'composer') {
      ST_composerPhoto = url;
    } else {
      const ok = ENGINE.SmallThings.updateEntry(c.target.id, { image: url });
      if (!ok) stNotify('That cut-out is too large to keep on this device — the whole photo was kept instead.');
    }
    closePhotoCutter();
    renderSmallThings();
  }
  ...
  function renderSmallThingsView(id) {
    ...
    $view().innerHTML = `
      <div class="card" data-od-id="smallthings-view">
        ...
        <div class="st-atlas">${entries.map(e => `
          <figure class="st-obj">
            ${e.image ? `<button class="st-obj-photobtn" type="button" data-act="smallthings-zoom" data-id="${esc(e.id)}" aria-label="View this photo larger"><img class="st-obj-photo" src="${e.image}" alt="A photo from your small things list"></button>` : ''}
            ${e.text ? `<figcaption class="st-obj-words">${esc(e.text)}</figcaption>` : ''}
            <span class="st-obj-time">${stHHMM(e.at)}</span>
          </figure>`).join('')}
        </div>
      </div>`;
    updateInspector();
  ...
  /* ---------- sampler cloth ---------- */
  /* Stitches sit directly on the cloth — no boxes, no backings, no borders.
     Each stitch is the user's own drawn pattern, rendered as running-stitch
     dashes in their chosen thread colour. Entries kept before drawn
     stitches existed fall back to the practice's legacy motif, also boxless. */
  const SAMP_THREADS = [
    { name: 'Sage', color: '#7d8c6f' },
    { name: 'Terracotta', color: '#b3563f' },
    { name: 'Indigo', color: '#5a6b8c' },
    { name: 'Charcoal', color: '#4a4640' }
  ];

  function stitchArtHTML(st) {
    const S = ENGINE.Sampler;
    if (st.pattern) return S.patternSVG(st.pattern);
    const m = S.motifFor(st.practiceId);
    return `<span class="samp-legacy" style="color:${m.color}" aria-hidden="true">${m.svg}</span>`;
  }

  function samplerClothHTML(stitches) {
    const S = ENGINE.Sampler;
    const cells = stitches.map(s => {
      const m = S.motifFor(s.practiceId);
      return `<button class="samp-stitch" type="button" data-act="sampler-detail" data-id="${esc(s.id)}" aria-label="Stitch: ${esc(m.title)}, ${esc(S.formatDate(s.at))}">${stitchArtHTML(s)}</button>`;
    }).join('');
    return `<div class="samp-cloth" role="list">${cells}</div>`;
  }
  ...
  function openStitchDetail(id) {
    ...
    ov.innerHTML = `
      <div class="samp-dialog">
        <div class="samp-swatch samp-weave" aria-hidden="true">${stitchArtHTML(st)}</div>
  ...
  function closeStitchDetail() {
    const ovs = $view().querySelectorAll('.samp-overlay');
    ovs.forEach(ov => { if (!ov.querySelector('.samp-pad')) ov.remove(); });
  }

  /* ---------- stitch drawing pad ---------- */
  /* The pad holds its own state on UI while open; nothing is stored until
     the user taps "Stitch it in" — the keep-rule, as everywhere else. */
  function padState() { return UI._stitchPad; }

  function openStitchPad() {
    const S = ENGINE.Sampler;
    const pid = ENGINE.Session.practiceId;
    if (!pid || !S.isKnownPractice(pid)) return;
    closeStitchPad();
    UI._stitchPad = { strokes: [], thread: SAMP_THREADS[0].color, drawing: null };
    const ov = document.createElement('div');
    ov.className = 'samp-overlay';
    ov.setAttribute('role', 'dialog');
    ov.setAttribute('aria-label', 'Draw your stitch');
    ov.innerHTML = `
      <div class="samp-dialog samp-pad">
        <div class="samp-dialog-title">Draw your stitch</div>
        <p class="support">Use your finger or cursor — whatever shape this practice felt like. It goes straight onto the cloth, with nothing around it.</p>
        <canvas class="samp-pad-canvas samp-weave" width="320" height="320" aria-label="Stitch drawing area"></canvas>
        <div class="samp-threads" role="group" aria-label="Thread colour">
          ${SAMP_THREADS.map((t, i) => `<button type="button" class="samp-thread${i === 0 ? ' sel' : ''}" data-act="sampler-pad-color" data-color="${t.color}" style="--thread:${t.color}" aria-label="${t.name} thread" aria-pressed="${i === 0 ? 'true' : 'false'}"></button>`).join('')}
        </div>
        <div class="btnrow">
          <button class="btn2" type="button" data-act="sampler-pad-save" disabled>Stitch it in</button>
          <button class="ghost" type="button" data-act="sampler-pad-clear" disabled>Clear</button>
          <button class="ghost" type="button" data-act="sampler-pad-cancel">Not now</button>
        </div>
        <div class="ghostrow"><button class="ghost" type="button" data-act="sampler-pad-mark">Or add a simple mark instead</button></div>
      </div>`;
    ov.addEventListener('click', e => { if (e.target === ov) closeStitchPad(); });
    $view().appendChild(ov);
    wirePadCanvas(ov.querySelector('.samp-pad-canvas'));
  }

  function closeStitchPad() {
    const ovs = $view().querySelectorAll('.samp-overlay');
    ovs.forEach(ov => { if (ov.querySelector('.samp-pad')) ov.remove(); });
    UI._stitchPad = null;
  }

  function padRefreshButtons() {
    const pad = padState();
    const ov = $view().querySelector('.samp-pad');
    if (!pad || !ov) return;
    const hasInk = pad.strokes.length > 0;
    ov.querySelector('[data-act="sampler-pad-save"]').disabled = !hasInk;
    ov.querySelector('[data-act="sampler-pad-clear"]').disabled = !hasInk;
  }
  ...
  function padDraw(cv) {
    ...
    const draw = (st) => {
      ...
      if (st.pts.length === 1) {
        const p = st.pts[0];
        ctx.beginPath();
        ctx.arc(p[0] / 100 * W, p[1] / 100 * W, 3.4 / 100 * W, 0, Math.PI * 2);
        ctx.fill();
        return;
      }
      ctx.lineWidth = 5 / 100 * W;
      ctx.setLineDash([7 / 100 * W, 5 / 100 * W]);
      ctx.beginPath();
      st.pts.forEach((p, i) => {
        const x = p[0] / 100 * W, y = p[1] / 100 * W;
        if (i) ctx.lineTo(x, y); else ctx.moveTo(x, y);
      });
      ctx.stroke();
      ctx.setLineDash([]);
    };
    pad.strokes.forEach(draw);
    if (pad.drawing) draw(pad.drawing);
  }

  function wirePadCanvas(cv) {
    const toPad = (e) => {
      const r = cv.getBoundingClientRect();
      return [
    ...
    cv.addEventListener('pointerdown', (e) => {
      ...
      e.preventDefault();
      try { cv.setPointerCapture(e.pointerId); } catch (_) {}
      pad.drawing = { color: pad.thread, pts: [toPad(e)] };
      padDraw(cv);
    });
    cv.addEventListener('pointermove', (e) => {
      const pad = padState();
  ...
  function padSave(markOnly) {
    const S = ENGINE.Sampler;
    const pad = padState();
    const pid = ENGINE.Session.practiceId;
    const runId = ENGINE.Session.runId;
    let pattern;
    if (markOnly) {
      const c = pad ? pad.thread : SAMP_THREADS[0].color;
      pattern = { strokes: [
        { color: c, pts: [[32, 32], [68, 68]] },
        { color: c, pts: [[68, 32], [32, 68]] }
      ]};
    } else {
      if (!pad || !pad.strokes.length) return;
      pattern = { strokes: pad.strokes };
    }
    S.addStitch(pid, runId, undefined, pattern);
    closeStitchPad();
  ...
  function init() {
    ...
    $view().addEventListener('click', (e) => {
      ...
      const acts = {
        ...
        'sampler-stitch': () => openStitchPad(),
        'sampler-pad-color': () => {
          const pad = padState();
          if (!pad) return;
          pad.thread = btn.getAttribute('data-color') || pad.thread;
          const ov = $view().querySelector('.samp-pad');
          if (ov) ov.querySelectorAll('.samp-thread').forEach(t => {
            const sel = t.getAttribute('data-color') === pad.thread;
        ...
        'sampler-pad-clear': () => {
          const pad = padState();
          if (!pad) return;
          pad.strokes = []; pad.drawing = null;
          const cv = $view().querySelector('.samp-pad-canvas');
          if (cv) padDraw(cv);
          padRefreshButtons();
        },
        'sampler-pad-save': () => padSave(false),
        'sampler-pad-mark': () => padSave(true),
        'sampler-pad-cancel': () => closeStitchPad(),
        ...
        'smallthings-cut-composer': () => { if (ST_composerPhoto) openPhotoCutter(ST_composerPhoto, { kind: 'composer' }); },
        'smallthings-cut-entry': () => {
          const open = ENGINE.SmallThings.getOpen();
          const e = open && open.entries.find(x => x.id === btn.getAttribute('data-id'));
          if (e && e.image) openPhotoCutter(e.image, { kind: 'entry', id: e.id });
        },
        'cutter-clear': () => cutterClearPath(),
        'cutter-keep-whole': () => closePhotoCutter(),
        'smallthings-close': () => {
