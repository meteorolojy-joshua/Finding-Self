...
const scenarioS = fs.readFileSync(path.join(__dirname, 'scenario.js'), 'utf8');
const testsS = fs.readFileSync(path.join(__dirname, 'scenario-tests.js'), 'utf8');
try {
  eval(engineS + ";\n" + scenarioS + ";\n" + testsS + "\n;run().then(() => { const pass = Results.filter(r=>r.pass).length; console.log('SUMMARY ' + pass + '/' + Results.length); Results.filter(r=>!r.pass).forEach(r=>console.log('FAIL: ' + r.label + (r.detail ? ' :: ' + r.detail : ''))); });");
