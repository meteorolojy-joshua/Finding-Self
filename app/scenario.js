...
window.SCENARIO = (() => {
  ...
  function migrate() {
    ...
    const templates = [
      ...
      { starter: 'SS02', id: 'SCN.DEFAULT.SS02', name: 'When Something Feels Off', std: 'NOW.OFF.S01', low: 'NOW.OFF.L01' },
      { starter: 'SS06', id: 'SCN.DEFAULT.SS06', name: "When I Can't Tell What I Want", std: 'NOW.UNCLEAR.S01', low: 'NOW.UNCLEAR.L01' }
    ];
    templates.forEach(t => {
      const existing = store.scenarios[t.id];
      if (existing) {
        // Premade check-ins keep their fixed names: refresh if an older
        // stored copy still carries a previous title.
        if (existing.origin && existing.origin.type === 'TEMPLATE' && existing.name !== t.name) {
          existing.name = t.name; existing.updated_at = now();
        }
        return;
      };
      const scenario = {
