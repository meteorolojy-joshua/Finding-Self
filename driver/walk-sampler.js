...
(async () => {
  ...
  // 2. stitch pad opens; draw a pattern with the mouse, then stitch it in
  ...
  await page.waitForTimeout(800);
  console.log('pad open:', await page.locator('.samp-pad').count() === 1);
  console.log('save disabled before drawing:', await page.locator('[data-act="sampler-pad-save"]').isDisabled());
  const cv = await page.locator('.samp-pad-canvas').boundingBox();
  const px = (fx, fy) => ({ x: cv.x + cv.width * fx, y: cv.y + cv.height * fy });
  await page.mouse.move(px(0.3, 0.3).x, px(0.3, 0.3).y);
  await page.mouse.down();
  for (let i = 1; i <= 8; i++) await page.mouse.move(px(0.3 + i * 0.05, 0.3 + Math.sin(i) * 0.15).x, px(0.3 + i * 0.05, 0.3 + Math.sin(i) * 0.15).y, { steps: 2 });
  await page.mouse.up();
  await page.waitForTimeout(300);
  console.log('save enabled after drawing:', await page.locator('[data-act="sampler-pad-save"]').isEnabled());
  await shot('s9-pad-drawn');
  await page.getByRole('button', { name: 'Stitch it in', exact: true }).click({ timeout: 8000 }).catch(e => console.log('SAVE CLICK FAILED', String(e).slice(0, 120)));
  await page.waitForTimeout(800);
  ...
  const stored = await page.evaluate(() => JSON.parse(localStorage.getItem('fsaw.sampler.v1') || '[]'));
  console.log('stitches in storage:', stored.length, '| has drawn pattern:', !!(stored[0] && stored[0].pattern && stored[0].pattern.strokes.length));
  await shot('s9-complete-screen');
  ...
  console.log('stitch boxless on cloth:', await page.locator('.samp-stitch svg').count() >= 1 && await page.locator('.samp-patch').count() === 0);
  ...
  // 5. stitch detail overlay (stitches are boxless .samp-stitch now)
  await page.click('.samp-stitch[data-act="sampler-detail"]', { timeout: 8000 }).catch(e => console.log('STITCH CLICK FAILED', String(e).slice(0, 120)));
  await page.waitForTimeout(800);
  ...
  await page.waitForTimeout(800);
  console.log('back to library:', (await text()).includes('Three practices are available'));
